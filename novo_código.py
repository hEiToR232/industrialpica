...
def desenhar_menu():
    ...
    # --- MOSTRAR TOP 5 ---
    desenhar_texto("TOP 5 JOGADORES", (largura//2 - 200, 180), BRANCO)
    y_offset = 230
    for i, (nome, pontos) in enumerate(top5, start=1):
        texto_ranking = fonte.render(f"{i}. {nome} - {pontos}", True, BRANCO)
        tela.blit(texto_ranking, (largura//2 - texto_ranking.get_width()//2, y_offset))
        y_offset += 40

    # BOTÃO JOGAR
    botao = pygame.Rect(largura//2 - 100, altura//2 + 100, 200, 60)
    pygame.draw.rect(tela, BRANCO, botao, 2)
    tela.blit(jogar, (largura//2 - jogar.get_width()//2, altura//2 + 100 + (60 - jogar.get_height())//2))
